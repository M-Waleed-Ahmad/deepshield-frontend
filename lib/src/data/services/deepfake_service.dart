...
class DeepfakeService {
  ...
  Future<AnalysisResult> analyze(DeepfakeRequest request) async {
    ...
    try {
      final streamed = await multipartRequest.send().timeout(const Duration(seconds: 60));
      final response = await http.Response.fromStream(streamed);
      
      if (response.statusCode == 400) {
        throw Exception('This file could not be analyzed. Accepted formats: JPG, PNG, WEBP, MP4, MOV, AVI.');
      }
      if (response.statusCode == 413) {
        throw Exception('File exceeds the 50MB limit. Please upload a smaller file.');
      }
      if (response.statusCode != 200) {
        throw Exception('Analysis failed. Please try again. Raw: ${response.reasonPhrase}');
      }
      ...
      throw Exception('Connection to the analysis server timed out. Please check your network and try again.');
    } catch (e) {
      if (e.toString().contains('This file could not be analyzed') || 
          e.toString().contains('exceeds the 50MB limit') || 
          e.toString().contains('Connection to the analysis server timed out') ||
